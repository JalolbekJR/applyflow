...
Status: in progress. Slices 1-8 are implemented for their approved technical scope: backend Slices
...
stabilization, frontend replacement documentation, and current-state reconciliation; Slice 8 adds
the manual expired-draft and private-storage cleanup command. Final Phase 3 handoff still requires
review of Slice 8 evidence, documentation reconciliation, manual accessibility review, and fresh
PostgreSQL runtime/concurrency verification after relevant backend changes before any
...
- Cleanup scheduling, production object-storage verification, and provider-specific
  eventual-consistency evidence.
- Manual accessibility review beyond automated and focused browser checks.
- Fresh PostgreSQL runtime/concurrency execution evidence after relevant backend changes.
...
| 8. Cleanup                       | Complete for approved scope: idempotent dry-run/batched management command, expired-draft revoke/scrub, terminal-shell preparation, pending blob deletion retry, stale-orphan grace period, safe hard deletion, item-failure exit behavior, and aggregate command output.                                                                                                                               | Slices 1, 4, and 6.                           | GPT-5.5                                                         | High; deletion failures must not leak data or lose cleanup keys.         | Dry-run/apply tests, repeated-run tests, storage-failure retries, missing-object completion, orphan grace tests, hard-delete eligibility, and privacy-output assertions.                                                                    |
| 9. Security and regression tests | Current Slices 1-8 security and regression coverage is implemented for approved scope. PostgreSQL concurrency tests are isolated behind a dedicated marker and settings module, cleanup scheduling is deferred, and final independent adversarial review is still required before handoff.                                                                                                              | Slices 1-8 for current evidence.              | GPT-5.5                                                         | High; independent adversarial review is needed before handoff.           | Current backend/frontend suites, Slice 7 browser checks, Slice 8 cleanup tests, and PostgreSQL-specific tests against an actual local or CI PostgreSQL service.                                                                             |
| 10. Documentation reconciliation | Current-state documentation reconciliation for Slices 1-8 is implemented, including frontend replacement documentation and cleanup-command documentation. Final Phase 3 documentation still needs a handoff pass after verification results and any approved PostgreSQL/accessibility evidence are known.                                                                                               | Current Slices 1-8 evidence.                  | GPT-5.4                                                         | Medium; accuracy and handoff consistency are primary.                    | Current link/style/stale-claim checks and `git diff --check` after cleanup lands; final handoff pass remains before production-readiness claims.                                                                                            |
...
4. **Storage gate:** Slice 4 covers private-root isolation, generated keys, duplicate-save
   ...
   replacement compensation and retryable physical deletion metadata. Slice 8 adds bounded pending
   deletion retry and stale-orphan cleanup through the storage adapter.
...
- Replacement preserves the previous active document until the new one succeeds; deletion and
  cleanup remain retryable through durable metadata and the cleanup command.
...
4. Review Slice 8 cleanup evidence before Phase 3 final handoff.
...
- Database and storage operations cannot be one transaction; compensation and orphan cleanup now
  have SQLite/fake-storage failure-injection tests, while production provider behaviour still needs
  separate evidence.
