...
| CV upload | Primary experience evidence. | Yes | File selected, size shown, PDF extension hint. | PDF extension allowlist, maximum 5 MiB, one file per request, file-signature and strict structural inspection, 1-10 page limit, active-content rejection, generated private storage name. | Upload-specific state plus summary. | Sensitive document. | Private server storage. | Never log contents, original name, or storage path. |
...
| Employment entries | Let candidates add concise context when a role is not obvious from the CV. | No; maximum five. | Required organization, role, and month range for each added entry. | Parent ownership, five-entry cap, month ordering, current-role consistency, lengths, and plain text. | Entry-level inline errors plus summary. | Personal/application data. | Server draft child records. | Do not log values. |
...
Do not place candidate personal data, draft secrets, status lookup secrets, application references, or CV metadata in localStorage or sessionStorage.
...
- Non-persistent in-memory UI preferences if needed, such as dismissed non-sensitive tips.
